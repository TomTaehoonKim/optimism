...
// NewMinimalEthClient creates a [sources.EthClient] with a minimal interface.
func NewMinimalEthClient(client client.RPC, log log.Logger, metrics caching.Metrics, config *sources.EthClientConfig) (MinimalEthClient, error) {
	ethClient, err := sources.NewEthClient(client, log, metrics, config)
	if err != nil {
		return nil, err
	}
